...
def prepare_wait_changes():
  ...
  with conn.cursor() as curaudit:
    curaudit.execute(
      "Select Concat(sales_orders.prefix, sales_orders.so_number) As Order_No, \
              ...
              sales_orders_audit.before_value_string as Waiting_Before_State, sales_orders_audit.after_value_string as Waiting_After_State, \
            ...
            Where sales_orders_cstm.neworexistingsystem_c In ('New', 'Existing') And \
              sales_orders_audit.field_name = 'waiting_on' And \
  ...
  if number_of_records:
    dicts = [{'order_no': r['Order_No'], 'project_name':r['name'] ,'order_date':r['date_entered'], 'order_category':r['order_category'],'Assigned_to':r['Assigned_to'] , 'order_value':r['GBP_Excl_Vat'] , 'Update_Date':r['Update_date'], \
              'Updated_by':r['Updated_by'],'Waiting_Before_State':r['Waiting_Before_State'],'Waiting_After_State':r['Waiting_After_State']} for r in records]
  ...
  app_tables.sales_orders_wait_changes.delete_all_rows()
  ...
  for row in dicts:
    ...
    app_tables.sales_orders_wait_changes.add_row(**{'order_no':row['order_no'],'project_name':row['project_name'], 'order_date':row['order_date'],'order_value':row['order_value'],'order_value_formated':row['order_value_formated'], \
                                                       'Update_Date':row['Update_Date'], 'order_category':row['order_category'], 'updated_by':row['Updated_by'],'waiting_before_state':row['Waiting_Before_State'],'waiting_after_state':row['Waiting_After_State']})                                         

    print('row loaded')
    for row in app_tables.sales_orders_wait_changes.search():
