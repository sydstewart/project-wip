...
class list_projects(list_projectsTemplate):
  def __init__(self, **properties):
    ...
    dicts, Xmedia ,pivotsyd = anvil.server.call('get_orders', self.percent_complete_text_box.text,self.drop_down_1.selected_value, self.Category.selected_value,self.invoiced_not_complete.selected_value)
  ...
  def percent_complete_text_box_change(self, **event_args):
    ...
    print('Assigned to =', self.drop_down_1.selected_value)
    print('Catgeory =', self.Category.selected_value)
    dicts,Xmedia , pivots = anvil.server.call('get_orders', self.percent_complete_text_box.text,self.drop_down_1.selected_value, self.Category.selected_value,self.invoiced_not_complete.selected_value)
    self.text_box_1.text = len(dicts)
    self.repeating_panel_1.items = dicts
    tallies(self, dicts)
